...
func (c *DPFMAPICaller) subfuncProcess(
	...
	for _, fn := range accepter {
		...
		switch fn {
		case "PriceMaster":
			c.priceMasterCreate(mtx, wg, subFuncFin, input, output, subfuncSDC, errs, log)
		default:
			wg.Done()
		}
	}
...
func (c *DPFMAPICaller) priceMasterCreate(
	mtx *sync.Mutex,
	wg *sync.WaitGroup,
	errFin chan error,
	input *dpfm_api_input_reader.SDC,
	...
	defer wg.Done()
	err = c.complementer.ComplementPriceMaster(input, subfuncSDC, log)
	if err != nil {
		mtx.Lock()
		*errs = append(*errs, err)
		mtx.Unlock()
